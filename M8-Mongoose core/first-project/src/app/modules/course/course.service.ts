...
const updateCoursesIntoDB = async (id: string, payload: Partial<TCourse>) => {
  ...
  try {
    ...
    const basicInfoUpdated = await Course.findByIdAndUpdate(
      id,
      remainingCourseData,
      { new: true, session },
    );
    ...
    if (preRequisiteCourses && preRequisiteCourses?.length > 0) {
      ...
      if (!deletedPreRequisiteCourse) {
        throw new AppError(
          ...
          'Failed to deletePreRequisite Course',
        );
      }
...
const assignFacultiesOnSingleCourseIntoDb = async (
  id: string,
...
const removeFacultiesOnSingleCourseIntoDb = async (
  id: string,
  payload: Partial<TCourseFaculties>,
) => {
  const result = await CourseFaculty.findByIdAndUpdate(
